...
from hjbgnn.utils.utils import jax_vmap

from .data import Rollout
from .utils import rollout, rollout_random
...
class Trainer:
    ...
    def train(self):
        ...
        def rollout_fn_single(params_actor, key):
            return rollout(self.env, lambda graph, key_: self.algo.step(graph, key_, params=params_actor), key)

        def rollout_fn(params_actor, keys):
            return jax.vmap(ft.partial(rollout_fn_single, params_actor))(keys)

        rollout_fn = jax.jit(rollout_fn)
        rollout_r_fn = jax.jit(jax.vmap(ft.partial(rollout_random, self.env)))

        # preprocess the test function
        def test_fn_single(params_actor, key):
            return rollout(self.env_test, lambda graph, k: (self.algo.act(graph, params_actor), None), key)

        def test_fn(params_actor, keys):
            return jax.vmap(ft.partial(test_fn_single, params_actor))(keys)
        ...
        for step in range(0, self.steps + 1):
            ...
            if step % self.eval_interval == 0:
                test_rollouts = test_fn(self.algo.actor_params, test_keys)
                ...
                finish = finish_fun(test_rollouts.graph).max(axis=1)
                cost = test_rollouts.costs.sum(axis=-1).mean()
                unsafe_frac = test_rollouts.costs.max(axis=-1) >= 1e-6
                safe_reaching =  finish & (~unsafe_frac[:, None])
                finish = finish.mean()
                unsafe_frac = np.mean(unsafe_frac)
                safe_reaching = safe_reaching.mean()
                eval_info = {
                    ...
                    "eval/finish": finish,
                    "eval/safe_reaching": safe_reaching,
                    "step": step,
                }
            ...
            key_x1, self.key = jax.random.split(self.key)
            key_x1 = jax.random.split(key_x1, self.n_env_train*3)
            rollouts_random = rollout_r_fn(key_x1)

            # update the algorithm
            update_info = self.algo.update(rollouts, rollouts_random, step)
