...
def rollout(
    ...
    init_graph = env.reset(key_x0, random=False)

    def body(graph, key_):
        action, log_pi = actor(graph, key_)
        next_graph, reward, cost, done, info = env.step(graph, action, key_)
...
def rollout_random(
    ...
    def body(i, key_):
        graph = env.reset(key_, random=True)
        i += 1
        return i, (graph)

    keys = jax.random.split(key, env.max_episode_steps)
    n, (graphs) = jax.lax.scan(body, 0, keys, length=env.max_episode_steps)
    data = Rollout(graphs, None, None, None, None, None, None)
