...
class DeterministicValue(nn.Module):
    _nu: int
    _n_state: int = None

    @nn.compact
    def __call__(self, obs: GraphsTuple, n_agents: int, *args, **kwargs) -> Action:
        e = obs.states[:n_agents,:] - obs.states[n_agents:2*n_agents,:]
        if self._n_state is not None:
            x = e[...,0:self._n_state]
        else:
            x = e
        x = nn.Dense(256, kernel_init=default_nn_init(), name="o4", use_bias=False)(x)
        x = nn.relu(x)
        x = nn.Dense(256, kernel_init=default_nn_init(), name="o3", use_bias=False)(x)
        x = nn.relu(x)
        x = nn.Dense(256, kernel_init=default_nn_init(), name="o2", use_bias=False)(x)
        x = nn.relu(x)
        x = nn.Dense(256, kernel_init=default_nn_init(), name="o1", use_bias=False)(x)
        x = nn.relu(x)
        x = nn.Dense(self._nu, kernel_init=default_nn_init(), name="OutputDense", use_bias=False)(x)
...
class MultiAgentValuePolicy(ABC):
    ...
    @abstractmethod
    def get_value(self, params: Params, obs: GraphsTuple) -> Action:
        pass

    @abstractmethod
    def sample_value(self, params: Params, obs: GraphsTuple, key: PRNGKey) -> Tuple[Action, Array]:
        pass

    @abstractmethod
    def eval_value(self, params: Params, obs: GraphsTuple, action: Action, key: PRNGKey) -> Tuple[Array, Array]:
        pass

class DeterministicValuePolicy(MultiAgentValuePolicy):
    ...
    def __init__(
        ...
        super().__init__(node_dim, edge_dim, n_agents, action_dim)
        self.net = DeterministicValue(_nu=action_dim, _n_state=_n_state)
        self.std = 0.1
    ...
    def sample_value(self, params: Params, obs: GraphsTuple, key: PRNGKey) -> Tuple[Action, Array]:
        action = self.get_value(params, obs)
        log_pi = jnp.zeros_like(action)
        return action, log_pi
