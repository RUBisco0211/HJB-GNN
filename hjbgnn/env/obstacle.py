...
CUBOID = jnp.ones(1)
SPHERE = jnp.ones(1) * 2
DYNAMICRECTANGLE = jnp.zeros(1) * 3
...
class DynamicRectangle(NamedTuple):
    type: ObsType
    center: Pos2d
    width: ObsWidth
    height: ObsHeight
    ...
    @staticmethod
    def create(center: Pos2d, width: ObsWidth, height: ObsHeight, theta: ObsTheta, center_goal: Pos2d = None, theta_goal: ObsTheta = None) -> "DynamicRectangle":
        bbox = jnp.array([
            [width / 2, height / 2],
            [-width / 2, height / 2],
            [-width / 2, -height / 2],
        ...
        return DynamicRectangle(DYNAMICRECTANGLE, center, width, height, theta, points, center if center_goal is None else center_goal, theta if theta_goal is None else theta_goal)
    ...
    def step(self, dt, key, area_size):
        # center
        error = self.center_goal - self.center
        norm_error = jnp.linalg.norm(error, axis=-1, keepdims=True)
        v_center = error / (norm_error+1e-8) * 0.1
        
        c_key, key = jr.split(key, 2)
        new_center_goal = jax.random.uniform(c_key, shape=self.center_goal.shape, minval=0, maxval=area_size)
        new_center_goal = jnp.where(jnp.abs(error)<0.1, new_center_goal, self.center_goal)
        
        # theta
        error = self.theta_goal - self.theta
        v_theta = error * 0.1
        
        t_key, key = jr.split(key, 2)
        new_theta_goal = jax.random.uniform(t_key, shape=self.theta_goal.shape, minval=-1, maxval=1)*jnp.pi
        new_theta_goal = jnp.where(jnp.abs(error)<0.1, new_theta_goal, self.theta_goal)
        
        return self.replace(center=self.center + v_center*dt, theta=self.theta + v_theta*dt, center_goal=new_center_goal, theta_goal=new_theta_goal)

    def replace(
        self,
        center: Pos2d = None,
        width: ObsWidth = None,
        height: ObsHeight = None,
        theta: ObsTheta = None,
        center_goal: Pos2d = None,
        theta_goal: ObsTheta = None,
    ) -> "DynamicRectangle":
        center_ = self.center if center is None else center
        width_ = self.width if width is None else width
        height_ = self.height if height is None else height
        theta_ = self.theta if theta is None else theta
        center_goal_ = self.center_goal if center_goal is None else center_goal
        theta_goal_ = self.theta_goal if theta_goal is None else theta_goal
        fun = jax_vmap(self.create)
        return fun(center_, width_, height_, theta_, center_goal_, theta_goal_)
