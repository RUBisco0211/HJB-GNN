...
from .obstacle import Cuboid, Sphere, Obstacle, Rectangle, DynamicRectangle
...
def get_obs_collection(
        obstacles: Obstacle, color: str, alpha: float, linewidth=5
):
    if isinstance(obstacles, Rectangle) or isinstance(obstacles, DynamicRectangle):
        n_obs = len(obstacles.center)
        obs_polys = [Polygon(obstacles.points[ii]) for ii in range(n_obs)]
        obs_col = PatchCollection(obs_polys, color=color, alpha=alpha, linewidths=linewidth, zorder=99)
...
def render_video(
    ...
    obs_col = get_obs_collection(obs, obs_color, alpha=alpha)
    ...
    if dim == 2:
        if show_edges:
            ...
            edge_col = LineCollection(e_lines, colors=e_colors, linewidths=0, alpha=0, zorder=3)
    else:
        if show_edges:
            edge_col = Line3DCollection(e_lines, colors=e_colors, linewidths=2, alpha=0.5, zorder=3)
        else:
            edge_col = Line3DCollection(e_lines, colors=e_colors, linewidths=0, alpha=0, zorder=3)
    ...
    if dim == 2:
        if show_text:
            ...
            cost_text = ax.text(0.02, 1.04, "", va="bottom", **text_font_opts)
    else:
        if show_text:
            cost_text = ax.text2D(0.02, 1.04, "Cost: 1.0, Reward: 1.0", va="bottom", **text_font_opts)
        else:
            cost_text = ax.text2D(0.02, 1.04, "", va="bottom", **text_font_opts)
    ...
    if Ta_is_unsafe is not None:
        if dim == 2:
            if show_text:
                ...
                safe_text = [ax.text(0.02, 1.00, "", va="bottom", **text_font_opts)]
        else:
            if show_text:
                safe_text = [ax.text2D(0.02, 1.00, "Unsafe: {}", va="bottom", **text_font_opts)]
            else:
                safe_text = [ax.text2D(0.02, 1.00, "", va="bottom", **text_font_opts)]

    # text for time step
    if dim == 2:
        if show_text:
            kk_text = ax.text(1.00, 1.00, "kk=0", va="bottom", ha="right", **text_font_opts)
        else:
            kk_text = ax.text(1.00, 1.00, "", va="bottom", ha="right", **text_font_opts)
    else:
        if show_text:
            kk_text = ax.text2D(1.00, 1.00, "kk=0", va="bottom", ha="right", **text_font_opts)
        else:
            kk_text = ax.text2D(1.00, 1.00, "", va="bottom", ha="right", **text_font_opts)
    ...
    if dim == 2:
        if show_text:
            agent_labels = [ax.text(n_pos[ii, 0], n_pos[ii, 1], f"{ii+1}", **label_font_opts) for ii in range(n_agent)]
        else:
            agent_labels = [ax.text(n_pos[ii, 0], n_pos[ii, 1], f"", **label_font_opts) for ii in range(n_agent)]
    else:
        for ii in range(n_agent):
            ...
            if show_text:
                ...
                agent_labels.append(ax.text2D(pos2d[0], pos2d[1], f"", **label_font_opts))
    ...
    if "cbf" in viz_opts:
        if dim == 3:
            ...
            cbar = fig.colorbar(cnt, ax=ax, shrink=0.8)
            ...
            if show_text:
                ax.text(0.5, 1.0, "CBF for {}".format(cbf_num+1), transform=ax.transAxes, va="bottom")

    # init function for animation
    def init_fn() -> list[plt.Artist]:
        return [agent_col, edge_col, *agent_labels, cost_text, *safe_text, *cnt_col, kk_text, obs_col]
    ...
    def update(kk: int) -> list[plt.Artist]:
        ...
        # update obstacles
        if isinstance(graph.env_states.obstacle, DynamicRectangle):
            n_obs = len(graph.env_states.obstacle.center)
            obs_polys = [Polygon(graph.env_states.obstacle.points[ii]) for ii in range(n_obs)]
            obs_col.set_paths(obs_polys)
        ...
        return [agent_col, edge_col, *agent_labels, cost_text, *safe_text, *cnt_col_t, kk_text, obs_col]
