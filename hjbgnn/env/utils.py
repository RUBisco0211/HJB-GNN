...
from scipy.linalg import inv, solve_discrete_are, solve_continuous_are
from typing import Callable, Tuple
from jax.lax import while_loop

from ..utils.typing import Array, Radius, BoolScalar, Pos, State, Action, PRNGKey
from ..utils.utils import merge01
from .obstacle import Obstacle, Rectangle, DynamicRectangle, Cuboid, Sphere
...
def get_lidar(start_point: Pos, obstacles: Obstacle, num_beams: int, sense_range: float, max_returns: int = 32):
    if isinstance(obstacles, Rectangle) or isinstance(obstacles, DynamicRectangle):
        thetas = jnp.linspace(-np.pi, np.pi - 2 * np.pi / (1e-8+num_beams), num_beams)
