...
class CrazyFlie(MultiAgentEnv):
    ...
    def __init__(
        ...
        self._R = np.eye(self.action_dim) * 1
        
        # high level param
        A = np.zeros((4,4))
        B = np.eye(4)
        Q = np.diag(np.array([1.0, 1.0, 1.0, 1.0]))*5
        R = np.diag(np.array([1.0, 1.0, 1.0, 1.0]))*1
        K, S, E = ct.lqr(A, B, Q, R)
        self._A_hl = jnp.array(A)
        self._B_hl = jnp.array(B)
        self._Q_hl = jnp.array(Q)
        self._R_hl = jnp.array(R)
        self._K_hl = jnp.array(K)
        # print("A:", A)
        # print("B:", B)
        # print("Q:", Q)
        # print("S:", S)
    ...
    def reset(self, key: Array, random=False) -> GraphsTuple:
        self._t = 0
        ...
        if random is False:
            ...
            s_key, key = jr.split(key, 2)
            # states = jnp.concatenate([states, jax.random.uniform(s_key, shape=(self.num_agents, self.state_dim - 3), minval=-self.area_size, maxval=self.area_size)], axis=1)
            _, up_lim = self.state_lim()
            up_lim = up_lim.at[3].set(self.area_size)
            states = jnp.concatenate([states, jax.random.uniform(s_key, shape=(self.num_agents, self.state_dim - 3), minval=-1, maxval=1)*up_lim[3:]], axis=1)
            goals = jnp.concatenate([goals, jnp.zeros((self.num_agents, self.state_dim - 3))], axis=1)
            states = states*2
            goals = goals*2
    ...
    def step(
        self, graph: EnvGraphsTuple, action: Action, key: Array, get_eval_info: bool = False
    ...
    def agent_xdot_hl(self, agent_states: AgentState, action: Action) -> AgentState:
        x_dot = ei.einsum(self._A_hl, agent_states[...,0:4],'m nx, batch nx->batch m') + ei.einsum(self._B_hl, action,'m nu, batch nu->batch m')
        return x_dot
    ...
    def get_graph(self, state: EnvState) -> GraphsTuple:
        ...
        node_type = node_type.at[self.num_agents: self.num_agents * 2].set(CrazyFlie.GOAL)
        node_type = node_type.at[-n_hits:].set(CrazyFlie.OBS)
