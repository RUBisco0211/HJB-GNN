...
from hjbgnn.algo import HJBGNN, make_algo
from hjbgnn.env import make_env
from hjbgnn.env.base import RolloutResult
from hjbgnn.trainer.utils import get_bb_cbf
from hjbgnn.utils.graph import GraphsTuple
from hjbgnn.utils.utils import jax_jit_np, tree_index, chunk_vmap, merge01, jax_vmap
from hjbgnn.env.plot import get_obs_collection
from hjbgnn.utils.utils import MutablePatchCollection
...
def test(args):
    ...
    if not args.u_ref:
        if args.path is not None:
            ...
            if args.cpu:
                act_fn = jax.jit(algo.act, device=device)
            else:
    ...
    for i_epi in range(args.epi):
        ...
        if args.nojit_rollout:
            ...
            #rollouts: Rollout = rollout_fn(self.algo.actor_params, self.algo.value_params, step, key_x0)
            # if not jnp.isnan(rollout.T_reward).any():
    ...
    is_unsafe = np.mean(is_unsafe,axis=1)
    is_finish = np.mean(is_finish,axis=1)
    ...
    for ii, (rollout, Ta_is_unsafe, cbf) in enumerate(zip(rollouts, is_unsafes, cbfs)):
        video_name = f"n{num_agents}_step{step}_epi{ii:02}_reward{rewards[ii]:.3f}_cost{costs[ii]:.3f}_seed{args.seed}"

        viz_opts = {}
        if args.cbf is not None:
            video_name += f"_cbf{args.cbf+1}"
